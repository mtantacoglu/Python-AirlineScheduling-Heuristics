...
# create a new DataFrame with zeros
    ...
    df_new_flights_original = df_new_flights.copy(deep = True)
    ...
    # check if the flight has available capacity and update the result accordingly
            if min_waiting_flight_index > 0 and (df_new_flights.loc[df_new_flights['New Flights'] == min_waiting_flight_index, 'Capacity'].values[0] > 0):
                ...
                if (df_new_flights.loc[df_new_flights['New Flights'] == min_waiting_flight_index, 'Capacity'].values[0] == 0):
                    ...
                    remaning_ratio = (df_new_flights.loc[df_new_flights['New Flights'] == min_waiting_flight_index, 'Capacity'].values[0])/(df_new_flights_original.loc[df_new_flights_original['New Flights'] == min_waiting_flight_index, 'Capacity'].values[0])
                    df_waiting[min_waiting_flight_index] = df_waiting[min_waiting_flight_index] * remaning_ratio
...
def heuristic(arrivals,readytime,scheduled,new,maximum_waiting_time_parameter_s,maximum_waiting_time_parameter_n):
    ...
    passenger_scheduled_flights.to_excel("passenger_scheduled_flights.xlsx", sheet_name="passenger_scheduled_flights")
    
    passenger_new_flights.to_excel("passenger_new_flights.xlsx", sheet_name="passenger_new_flights")
